...
const parsedStocksInfos = computed(() => {
  const minLength = Math.min(...stocksInfos.value.map(stockInfo => stockInfo.data.length))
      ...
      y: {
        formatter: function (val: number, data: any) {
          ...
          const pctChange = Number(
            ((series[seriesIndex][dataPointIndex] - series[seriesIndex][0]) / series[seriesIndex][0]) * 100,
          )
          return '$' + val.toFixed(2) + ' ' + (pctChange > 0 ? '+' : '') + pctChange.toFixed(2) + '%'
        },
      },
    },
    grid: {
...
<template>
  <VCard>
    <VCardTitle class="text-h5">Stocks Performance</VCardTitle>
    <VCardText>
      <VueApexCharts
        type="line"
